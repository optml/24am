...
template<typename F>
void executeSolver(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	//associate outputs
	if (sizeof(F) == 4) {
		x_out_n = plhs[0] = mxCreateNumericArray(1, &dim_n, mxSINGLE_CLASS,mxREAL);
	} else {
	...
	if (sizeof(F) == 4) {
		optimizationSettings->useDoublePrecision = false;
	} else {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	mxArray *B_mat;
	B_mat = mxDuplicateArray(prhs[0]);
	bool isDoublePrec = mxIsDouble(B_mat);
	if (isDoublePrec) {
		executeSolver<double>(nlhs, plhs, nrhs, prhs);
	} else {
		executeSolver<float>(nlhs, plhs, nrhs, prhs);
	}
}
