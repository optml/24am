...
int denseDataSolver(cublasHandle_t &handle, const unsigned int m,
	...
	double total_memory = LD_N * LD_M + LD_N * optimizationSettings->starting_points * 2
			+ LD_M * optimizationSettings->starting_points;
	total_memory = total_memory * sizeof(F)
			+ LD_N * optimizationSettings->starting_points * 4;
	...
	thrust::device_vector<F> d_z(LD_M * optimizationSettings->starting_points, 0);
	if (!optimizationSettings->isConstrainedProblem()) {
		...
		thrust::transform(thrust::make_counting_iterator < F > (0),
				thrust::make_counting_iterator < F
						> (LD_M * optimizationSettings->starting_points), d_z.begin(),
				generator);
	}
	//alocate vector "x" on device
	thrust::device_vector<F> d_x(n, 0);
	thrust::device_vector<F> d_V(LD_N * optimizationSettings->starting_points, 0);
	...
	if (optimizationSettings->isConstrainedProblem()) {
		if (low_memory) {
			...
			d_IDX.resize(optimizationSettings->starting_points * LD_N, 0);
			thrust::transform(thrust::make_counting_iterator < F > (0),
					thrust::make_counting_iterator < F
							> (LD_N * optimizationSettings->starting_points), d_IDX.begin(),
					init_sequence_with_LDN(LD_N));
			dataToSort.resize(LD_N * optimizationSettings->starting_points);
		}
		for (int i = 0; i < optimizationSettings->constrain; i++) {
			for (int j = 0; j < optimizationSettings->starting_points; j++) {
	...
	for (unsigned int it = 0; it < optimizationSettings->max_it; it++) {
		if (optimizationSettings->isConstrainedProblem()) {
			...
			gpu_matrix_matrix_multiply(handle, CUBLAS_OP_N, ONE, m, n, B, V, z,
					optimizationSettings->starting_points, LD_M, LD_N);
